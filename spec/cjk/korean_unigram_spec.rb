...
describe "Korean: Unigram Searches", :korean => true do
  ...
  context "title  창 (window)" do
    it_behaves_like "expected result size", 'title', '창', 1300, 1450
    before(:all) do
...
#      @resp.should include({'vern_title_245a_display' => /창/}).in_each_of_first(9).documents
  ...
  context "title  꿈 (dream)" do
    it_behaves_like "expected result size", 'title', '꿈', 400, 500
    before(:all) do
      @resp = solr_response(cjk_query_args('title', '꿈').merge({'fq' => '-access_facet:Online', 'fl'=>'id,vern_title_245a_display', 'facet'=>false, 'rows'=>25} ))
    end
    it "titles should match regex" do
      expect(@resp).to include({'vern_title_245a_display' => /꿈/}).in_each_of_first(25).documents
    end
  end
