describe 'advanced search' do
  ...
  context 'subject and keyword' do
    ...
    context 'subject -congresses, keyword IEEE xplore', jira: 'SW-623' do
      ...
      it 'keyword' do
        ...
        expect(resp.size).to be >= 23_500
        expect(resp.size).to be <= 26_700
    ...
    context 'subject home schooling, keyword Socialization', jira: 'VUF-1352' do
      ...
      it 'subject not as a phrase' do
        expect(@sub_no_phrase.size).to be >= 960
        expect(@sub_no_phrase.size).to be <= 1330
      ...
      it 'subject as a phrase' do
        expect(@sub_phrase.size).to be >= 750
        expect(@sub_phrase.size).to be <= 950
      ...
      it 'keyword' do
        ...
        expect(resp.size).to be >= 800_000
        expect(resp.size).to be <= 860_000
      end
  ...
  context 'author + keyword' do
    context 'author campana, keywords storia e letteratura', jira: 'VUF-2468' do
      ...
      it 'author' do
        ...
        expect(@author_resp.size).to be <= 350
      ...
      it 'keyword' do
        ...
        expect(resp.size).to be >= 2550
        expect(resp.size).to be <= 2750
      end
  ...
  context 'pub info' do
    ...
    context "subject 'soviet union and historiography' and pub info '1910-1911", jira: 'VUF-1781' do
      ...
      it 'pub info 2010' do
        ...
        expect(resp.size).to be >= 200_000
        expect(resp.size).to be <= 220_000
      end
      it 'pub info 2011' do
  ...
  context 'facets' do
    ...
    context 'format video, location Media Microtext, language english' do
      ...
      it 'before topics selected' do
        resp = solr_resp_doc_ids_only({ 'fq' => 'format_main_ssim:("Video"), language:("English"), building_facet:("Media Center")', 'q' => 'collection:*' }.merge(solr_args))
        expect(resp.size).to be >= 37_000
        expect(resp.size).to be <= 47_000
      end
      it 'add topic feature films' do
        resp = solr_resp_doc_ids_only({ 'fq' => 'format_main_ssim:("Video"), language:("English"), building_facet:("Media Center"), topic_facet:("Feature films")', 'q' => 'collection:*' }.merge(solr_args))
        expect(resp.size).to be >= 9_300
        expect(resp.size).to be <= 10_500
      end
      it 'add topic science fiction' do
        resp = solr_resp_doc_ids_only({ 'fq' => 'format_main_ssim:("Video"), language:("English"), building_facet:("Media Center"), topic_facet:("Feature films"), topic_facet:("Science fiction films")', 'q' => 'collection:*' }.merge(solr_args))
        expect(resp.size).to be >= 200
        expect(resp.size).to be <= 300
      end
      it 'use genre feature films' do
        resp = solr_resp_doc_ids_only({ 'fq' => 'format_main_ssim:("Video"), language:("English"), building_facet:("Media Center"), genre_ssim:("Feature films")', 'q' => 'collection:*' }.merge(solr_args))
        expect(resp.size).to be >= 14_100
        expect(resp.size).to be <= 16_100
      end
      it 'add genre science fiction' do
        resp = solr_resp_doc_ids_only({ 'fq' => 'format_main_ssim:("Video"), language:("English"), building_facet:("Media Center"), genre_ssim:("Feature films"), genre_ssim:("Science fiction films")', 'q' => 'collection:*' }.merge(solr_args))
        expect(resp.size).to be >= 450
