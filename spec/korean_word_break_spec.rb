...
describe "Korean: Word Breaks", :korean => true, :fixme => true do
  ...
  it "한국경제 (no spaces) should retrieve all the records for  한국  경제 (with spaces)" do
    ...
    resp.should have_the_same_number_of_results_as(solr_resp_doc_ids_only({'q'=>'한국  경제'}).size) # 407 in prod, 495 in soc
  end
  ...
  it "한국  경제 should should retrieve all the records for  한국경제 (no space)" do
    ...
    resp.should have_the_same_number_of_results_as(solr_resp_doc_ids_only({'q'=>'한국경제'}).size) # 407 in prod, 495 in soc
  end
  ...
  context "search for record 9323348 with title  한국 경제 의 미래 와 생존 전략 " do
    ...
    it "한국경제의  미래와  생존전략 (2 spaces) should get results" do
      ...
      resp.should include(["9323348"]) 
      resp.should have_at_least(solr_resp_doc_ids_only({'q'=>' 미래와  생존전략'}).size).results # 0 in prod, 1 in soc
    end
