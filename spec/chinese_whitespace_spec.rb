...
describe "Chinese Searching - Extraneous Spaces (from old RLIN records?)" do
  ...
  it "whitespace variants of 三国演义 should get similar results" do
    resp = solr_resp_doc_ids_only({'q'=>'三 国演义'})
    resp.should have_at_least(100).documents
    resp.should have_at_most(125).documents
    resp.size.should be_within(5).of(solr_resp_doc_ids_only({'q'=>'三国  演义'}).size)
    resp.size.should be_within(5).of(solr_resp_doc_ids_only({'q'=>'三国 演义'}).size)
    resp.size.should be_within(5).of(solr_resp_doc_ids_only({'q'=>'三  国  演  义'}).size)
  end
